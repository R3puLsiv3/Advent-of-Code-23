...
def main():
    ...
    time_saves = {}
    for (row, col) in path:
        cheat_reachable = [(row + i,  col + j) for i in range(-20, 21) for j in range(-20, 21) if abs(i) + abs(j) <= 20]
        for reachable_row, reachable_col in cheat_reachable:
            if lines[reachable_row][reachable_col] != "#":
                time_saved = times[(reachable_row, reachable_col)] - times[(row, col)] - abs(row - reachable_row) - abs(col - reachable_col)
                if time_saved > 0:
